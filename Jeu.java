...
public class Jeu {
    ...
    public void tour(int direction){
        ...
        System.out.println("Vous ne pouvez pas déplacer cette pie�ce!");
        }
        else {
        ...
        //Ve�rification qu'une montagne hors dans le plateau. La fae�on de manipuler les pie�ces doit e�tre discute�e
       ...
       public void deplacerToutesPieces(Coordonnees c, int d){
        ...
        if(mouvementPossible(plateau[h][v],d)){
            while (plateau[h][v] instanceof Piece && estDansLePlateau(plateau[h][v])){ // permet de monter jusqu'a� la dernière case à déplacer
                switch (d)
                {
                    case 1: //nord
                        v--;
